...
function renderItem({ target, value, id, isCompleted = false, isInit = false }) {
...
function renderSchedule({ target, value, id, isCompleted = false, isInit = false }) {
...
function updateStats() {
  ...
  const completedCount = todoListArray.filter(item => item.isCompleted).length;
  ...
  if (completionChart) {
    completionChart.data.datasets[0].data = [completionRate, 100 - completionRate];
